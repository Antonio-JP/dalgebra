...
def GetCentralizer(
    ...
    permutation = list(zip(*sorted(((op, i) for i,op in enumerate(operators) if op is not None), key=lambda op : op[0].order(z))))[1]
...
#################################################################################################
@lru_cache
def module_quo_rem(P: DPolynomial, L: DPolynomial, basis: tuple[DPolynomial], gen: DPolynomialGen) -> tuple[tuple[tuple[Element]], DPolynomial]:
    r'''
        Compute the `C[L]`-module quotient of a polynomial `P`.

        Given a linear differential operator `L` in normal form for whom we have computed a `C[L]`-basis of
        a `C[L]`-module (given with ``basis``), we can compute for any differential operator `P` two
        new polynomials `Q` and `R` such that `Q` is in the `C[L]`-module generated by `basis` and `R`
        can not be further reduced by the `C[L]`-module generated by `basis`.

        This can be used to check whether the element `P` belongs to the `C[L]`-module generated by `basis`.

        INPUT:

        * ``P``: the polynomial to be reduced.
        * ``L``: the operator that we are building the `C[L]`-module.
        * ``basis``: the basis of the `C[L]`-module as a tuple of differential operators.
    ...
    ## We go from the highest order of `P` removing the leading coefficient using the basis (if possible)
    CR = L.parent().constant_ring()
    R = L.parent().zero()
    output = [dict() for _ in basis] # Each dictionary will keep the coefficients for each basis element.

    while P.order(gen) >= L.order(gen):
        order = P.order(gen)
        logger.debug(f"[MQR] Remaining polynomial: {P} (order {order})")
        cong = order % L.order(gen)  # We see which element in basis to use
        if cong not in orders_cong or order < basis[orders_cong.index(cong)].order(gen):
            logger.debug(f"[MQR] The polynomial {P} is not in the `C[L]`-module generated by the basis")
            leading_term = P.coefficient_full(gen[order])*gen[order]
            R += leading_term
            P -= leading_term # this reduces the order of `P`
        else: # we can reduce using the basis
            i = orders_cong.index(cong) # we see which element in the basis to use
            n = (order - basis[i].order(gen)) // L.order(gen)  # This is the power of L necessary to generate the corresponding order ( >= 0 )

            output[i][n] = output[i][n] + CR(P.coefficient_full(gen[order])) if n in output[i] else CR(P.coefficient_full(gen[order]))
            T = P.coefficient_full(gen[order]) * (L.sym_power(n, gen).dot(basis[i], gen))  # This is the term we are removing
...
def BC_ideal(L: DPolynomial, basis: tuple[DPolynomial], gen: DPolynomialGen, *, var_L: str = "lambda_", var_B: str = "mu") -> Ideal:
    ...
    basis_operators = (gen[0],) + tuple(el if not isinstance(el, (tuple,list)) else el[1] for el in basis)

    ## Creating the final polynomial ring with as many elements as those that do not appear in known_relations
    names_mu, weights = zip(*[(f"{var_B}_{i}", basis_operators[i].order(gen)) for i in range(1,len(basis)+1)])
    ...
    l = final_ring.gens()[-1]
    mu = (final_ring.one(),) + final_ring.gens()[:-1]  # mu[i] is the variable for the i-th element of the basis (i > 0)

    ## 3. Generate all cross products of the basis and reduce module C[L]
    prods = {(i,j) : basis_operators[i].dot(basis_operators[j], gen) for i in range(1,len(basis_operators)) for j in range(i,len(basis_operators))}
    ## 4. Add these cross-product relations to the relations
    output = list()
    for (i,j), prod in prods.items():
        red_ij = reduce_as_module(prod, L, basis_operators, gen) # polynomial representation - no error possible
