...
r'''
    ...
    * The implementation is integrated properly in SageMath structures.
    * The implementation is integrated properly in all the structures of `dalgebra`.
    
    TODO (unassigned): another idea that could be acceptable is to provide full coverage of extensions not only on differential extensions
    (as is Bronstein's book) but also in the difference case.

    TODO (unassigned): we would also need to work with skew-derivations, but that is for a really long future.
'''
