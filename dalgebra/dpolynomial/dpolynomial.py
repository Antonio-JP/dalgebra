...
class DPolynomialRingFactory(UniqueFactory):
    r'''
        ...
        :class:`DPolynomialRing_new` for further information on this structure.
    '''
    ...
    def create_object(self, _, key) -> DPolynomialRing_new:
        base, names = key

        return DPolynomialRing_new(base, names)

DPolynomialRing = DPolynomialRingFactory("dalgebra.dpolynomial.dpolynomial_ring.DPolynomialRing_new")
RWOPolynomialRing = DPolynomialRing #: alias for DPolynomialRing_new (used for backward compatibility)
def DifferentialPolynomialRing(base, *names : str, **kwds) -> DPolynomialRing_new:
    if not base in _DRings:
    ...
    return DPolynomialRing_new(base, *names, **kwds)
def DifferencePolynomialRing(base, *names : str, **kwds) -> DPolynomialRing_new:
    if not base in _DRings:
...
class DMonomial:
    ...
    '''
    def __init__(self, parent: DPolynomialRing_new, input: dict[tuple[int,tuple[int]|int],int] | tuple[tuple[int,tuple[int]|int]|int, int]):
    ...
    def parent(self) -> DPolynomialRing_new:
    ...
    def __derivative__(self, operation: int = 0) -> tuple[DMonomial]:
        ...
        '''
        result = list()
        for (v, o), e in self.__variables.items():
            copy = self.__variables.copy()
            od = list(o); od[operation] += 1; od = tuple(od)
            copy[(v, od)] = self.__variables.get((v,o), 0)
            if e == 1: copy.pop((v, o))
            else: copy[(v,o)] = e - 1
            result.append((DMonomial(copy), e))
    ...
    def __hash__(self) -> int: #: hashing the dictionary using the tuples of items
        return hash(tuple(self.__variables.items()))
    ...
    def __eq__(self, other: DMonomial) -> bool:
        ...
        elif other in self.parent(): # case for elements of parent
            return self.parent()(other).__eq__(self)
        else:
            return False
    ...
    def eval(self, *args, dic : dict = None, **kwds):
        ...
        if len(args) != 0: 
            ...
            kwds.update({self.parent().gen(i).variable_name() : args[i] for i in range(len(args))})
        elif dic != None:
        ...
        for variable, value in kwds.items():
            index = self.parent().index_variable(variable)
            for (v,o), e in self.__variables.items():
                if v == index:
                    result *= (self.parent().apply_operations(value, o, _ordered=False))**e
                    copy.pop((v,o)) # this variable will not appear in the remaining monomial
        return result * (DMonomial(copy) if len(copy) > 0 else result.parent().one())

    @cached_method
    def __repr__(self) -> str:
        variable_names = [g.variable_name() for g in self.parent().gens()]
        elements = sorted(self.__variables.items())
...
class DPolynomialRing_new:
    r'''
        ...
        * ``names``: set of names for the variables of the new ring.
        ...
        Hence, we can create the ring of operator polynomials with as many variables as we want::
            ...
            sage: OR.<u,v> = DPolynomialRing_new(dsR); OR
    ...
    def __init__(self, base : Parent, names : Collection[str]):
        ...
        if not base.all_operators_commute():
            raise TypeError("Detected operators that do NOT commute. Impossible to build the DPolynomialRing_new")
    ...
    def gen(self, i: int | str = None) -> DPolynomialGen:
        r'''
            ...
            For a :class:`DPolynomialRing_new`, the generator type is 
            ...
            * ``i``: index or name of the required generator.
            ...
            EXAMPLES::
                ...
                sage: R.<y> = DPolynomialRing_new(DifferentialRing(QQ['x'], diff))
                sage: R.gen(0)
                y_*
                sage: R.gen(0) is y
                True
                ...
                True
                sage: S = DPolynomialRing_new(DifferentialRing(ZZ, lambda z : 0), ('a', 'b'))
                sage: S
                Ring of operator polynomials in (a, b) over Differential Ring [[Integer Ring], (0,)]
                sage: S.gen(0)
                a_*
        ...
        r'''
            ...
            * ``polynomial``: an element in ``self`` to be flatten.
            ...
            :class:`IndexBijection`::
                ...
                sage: R.<u,v> = DPolynomialRing_new(DSB)
    ...
    def change_ring(self, R):
        r'''
            ...
            * ``R``: a Ring with Operators.
            ...
            A :class:`DPolynomialRing_new` over ``R`` with the same variables as ``self``.
        '''
        return DPolynomialRing_new(R, self.variable_names())

    def append_variables(self, *variables) -> DPolynomialRing_new:
    ...
    def eval(self, element, *args, dic: dict[DPolynomialGen,DPolynomial] = None, **kwds):
        r'''
            ...
            * There are duplicate entries for a generator.
            ...
            As expected, similar behavior occurs when having several operators in the ring::
                ...
                sage: T.<u> = DPolynomialRing_new(DifferenceRing(DifferentialRing(QQ[x],diff), QQ[x].Hom(QQ[x])(QQ[x](x)+1))); x = T.base()(x)
        ...
        R = DPolynomialRing_new(self.base(), rem_names) if len(rem_names) > 0 else self.base()
    ...
    def add_constants(self, *new_constants: str) -> DPolynomialRing_new:
        return DPolynomialRing_new(self.base().add_constants(*new_constants), self.variable_names())
        ...
        r'''
            ...
            * ``gen``: an infinite variable that will be eliminated from `P` and `Q`. Can be ``None`` only if one infinite variable is in ``self``.
            ...
            EXAMPLES::
                ...
                sage: S.<z> = DPolynomialRing_new(B)
            ...
            If several variables are available, we need to explicitly provide the variable we are considering::
                ...
                sage: R.<u,v> = DPolynomialRing_new(B)
    ...
    def as_linear_operator(self, element: DPolynomial) -> Element:
        r'''
            ...
            Conversely, a :class:`DPolynomialRing_new` can transform elements from its ring of linear operators
            ...
            * ``element``: a :class:`DPolynomial` in ``self`` to be casted to a linear operator.
            ...
            This also work when having several operators in the same ring::
                ...
                sage: S.<u> = DPolynomialRing_new(DifferenceRing(DifferentialRing(QQ[x], diff), QQ[x].Hom(QQ[x])(QQ[x](x)+1))); x = S.base()(x)
...
def is_DPolynomialRing(element):
    ...
    '''
    return isinstance(element, DPolynomialRing_new)
...
class DPolynomialToLinOperator (Morphism):
    ...
    '''
    def __init__(self, dpoly_ring : DPolynomialRing_new):
...
class WeightFunction(SetMorphism):
    ...
    '''
    def __init__(self, dpoly_ring: DPolynomialRing_new, base_weights: list[int] | tuple[int] | dict[str|DPolynomialGen, int], operator_weights: list[int] |tuple[int]):
...
class RankingFunction:
    r'''
        ...
        * ``ttype``: the type of ranking to be created. Currently, only "elimination" or "orderly" are allowed:
        ...
        TODO: add examples
    '''
    def __init__(self, parent: DPolynomialRing_new, ordering: list[DPolynomialGen] | tuple[DPolynomialGen] = None, ttype: str = "orderly"):
        ## Processing arguments
        if not isinstance(parent, DPolynomialRing_new):
