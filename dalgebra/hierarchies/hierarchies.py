r'''
    ...
    **Theory explanation**
    ...
    .. MATH::
        ...
        L = \partial^n + u_{2}\partial^{n-2}  + \ldots + u_{n-1}\partial + u_0.
    ...
    * `P_m` almost commutes with `L`.
    ...
    Now we can compute the commutator `[L, P]` and, then create a system of differential equations with the highest order coefficients of the commutator::
        ...
        sage: system
        System over [Ring of operator polynomials in (p2, p3, p4, p5, u3, u2, z) over Differential Ring [[Rational Field], (0,)]] with variables [(p2_*, p3_*, p4_*, p5_*)]:
        {
            (-3)*p2_0*u3_1 + (-3)*p2_0*u2_2 + p3_1*u2_0 + (-2)*p3_0*u2_1 + p3_3 + 3*p4_2 + 3*p5_1 + (-10)*u3_3 + (-5)*u2_4 == 0
            p2_1*u2_0 + (-3)*p2_0*u2_1 + p2_3 + 3*p3_2 + 3*p4_1 + (-10)*u3_2 + (-10)*u2_3 == 0
            3*p2_2 + 3*p3_1 + (-5)*u3_1 + (-10)*u2_2 == 0
            3*p2_1 + (-5)*u2_1 == 0
        }
    ...
    variables `p_i` in term of `u_i`::
        ...
        5/3*u3_0 + 5/3*u2_1
        sage: sols[p[2]] # p4
        5/9*u2_0^2 + 5/3*u3_1 + 10/9*u2_2
        sage: sols[p[3]] # p5
        10/9*u3_0*u2_0 + 10/9*u3_2
    ...
    `n-2 = 1`. The last two coefficients that remain will be conditions on the differential variables `u_2, u_3`::
        ...
        5/9*u3_1*u2_0^2 + 10/9*u3_0*u2_1*u2_0 + 5/9*u3_3*u2_0 + (-5/3)*u3_2*u3_0 + 5/3*u3_2*u2_1 + 
        (-5/3)*u3_1^2 + 20/9*u3_1*u2_2 + 10/9*u3_0*u2_3 + 1/9*u3_5
        sage: C_eval.coefficient(z[1])
        5/9*u2_1*u2_0^2 + (-10/3)*u3_1*u3_0 + 5/3*u3_1*u2_1 + 5/3*u3_0*u2_2 + 5/9*u2_3*u2_0 + 5/9*u2_2*u2_1 + 1/9*u2_5
    ...
    **Special hierarchies**
    ...
    constant coefficient in `[L,P_m]` and the coefficient of ``z[1]``::
        ...
        5/9*u3_1*u2_0^2 + 10/9*u3_0*u2_1*u2_0 + 5/9*u3_3*u2_0 + (-5/3)*u3_2*u3_0 + 5/3*u3_2*u2_1 + (-5/3)*u3_1^2 + 20/9*u3_1*u2_2 + 10/9*u3_0*u2_3 + 1/9*u3_5
        5/9*u2_1*u2_0^2 + (-10/3)*u3_1*u3_0 + 5/3*u3_1*u2_1 + 5/3*u3_0*u2_2 + 5/9*u2_3*u2_0 + 5/9*u2_2*u2_1 + 1/9*u2_5
...
def __names_variables(order:int, var_name: str, *, simplify_names: bool = True) -> list[str]:
    r'''
        ...
        EXAMPLES::
            ...
            sage: from dalgebra.hierarchies.hierarchies import __names_variables
...
def generic_normal(n: int, 
    ...
    r'''
        ...
        * ``simplify_names`` (optional): used for the argument ``simplify_names`` in method :func:`__names_variables`.
        ...
        EXAMPLES::
            ...
            sage: from dalgebra.hierarchies.hierarchies import generic_normal
            sage: generic_normal(2)
            u_0*z_0 + z_2
            sage: generic_normal(3)
            u3_0*z_0 + u2_0*z_1 + z_3
            sage: generic_normal(4)
            u4_0*z_0 + u3_0*z_1 + u2_0*z_2 + z_4
            sage: generic_normal(4, "a")
            a4_0*z_0 + a3_0*z_1 + a2_0*z_2 + z_4
            sage: generic_normal(4, "a", "p")
            a4_0*p_0 + a3_0*p_1 + a2_0*p_2 + p_4
    ...
    try:
        output_z = output_ring.gen('z'); output_u = [output_ring.gen(name) for name in names_u] # output_u = [u2, u3, ..., un]
    except ValueError:
...
def GetEquationsForSolution(n: int, m: int, U: list | dict, extract, flag_name = "c", **kwds):
    r'''
        ...
        * ``n``: the order of the basic operator `L` (see :func:`almost_commuting_wilson`).
        * ``m``: the order bound of the commutator to look (see :func:`almost_commuting_wilson`).
